...
export async function generateMetadata({
  ...
  Object.keys(messagesMap).forEach((loc) => {
    const path = loc === "en" ? "/voyeurcam" : `/${loc}/voyeurcam`;
  ...
  let pageUrl = `${SITE_URL}/voyeurcam`;
  if (pathname && pathname !== "/") {
    pageUrl = `${SITE_URL}${pathname}`;
  } else {
    pageUrl = locale === "en" ? `${SITE_URL}/voyeurcam` : `${SITE_URL}/${locale}/voyeurcam`;
  }
  ...
  const ogImageUrl = getTopSiteImageUrl("voyeur", IMAGE_URL);
